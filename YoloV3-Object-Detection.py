...
# image Pro-pressesing functions :
...
def conv_block(inp, convs, skip=True):
    ...
    for conv in convs:
        ...
        if conv['stride'] > 1:  x = ZeroPadding2D(((1, 0), (1, 0)))(
            x)  # peculiar padding as darknet prefers left and top
        x = Conv2D(conv['filter'],
                   conv['kernel'],
                   strides=conv['stride'],
                   padding='valid' if conv['stride'] > 1 else 'same',
                   # peculiar padding as darknet prefers left and top
...
# load the weights
weight_reader = Read_Weights("WeightsUrlPath/yolov3.weights")
# set the weights
...
image_names1 = [ "FolderUrlPath/vache.jpg"]
#image_names2 = [ "FolderUrlPath/dog.jpg"]

#Detection
predict_boxes(image_names1)
#predict_boxes(image_names2)